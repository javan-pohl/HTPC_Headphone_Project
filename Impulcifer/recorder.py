...
def record_target(file_path, length, fs, channels=2, append=False):
    ...
    Returns:
        None
    """
    print(f"Recording to: {os.path.abspath(file_path)} | fs={fs} Hz | length={length} samples | channels={channels}")
    recording = sd.rec(length, samplerate=fs, channels=channels, blocking=True)
    recording = np.transpose(recording)
    
    # Calculate gain safely, avoiding divide by zero
    max_amplitude = np.max(np.abs(recording))
    if max_amplitude > 0:
        max_gain = 20 * np.log10(max_amplitude)
        print(f"Headroom: {0 - max_gain:.1f} dB")
    else:
        print("⚠️  WARNING: No audio recorded (silent)")
        max_gain = float('-inf')
    ...
    try:
        ...
        print(f"❌ ERROR: Failed to write WAV file '{file_path}': {ex}")
        return
...
def play_and_record(
    ...
    # For basic setups, allow any device (min_channels=0) if no specific devices are given
    min_channels_required = 0 if (input_device is None and output_device is None) else n_channels
    
    input_device, output_device = get_devices(
        input_device=input_device,
        output_device=output_device,
        host_api=host_api,
        min_channels=min_channels_required
    )
    ...
    # Handle both mono and stereo sweep files for playback
    # For recording, use the actual microphone channel count (typically 2 for binaural mics)
    recording_channels = 2  # Binaural microphones are stereo
    print(f'Recording channels (mics): {recording_channels}')
    
    # Calculate recording length in samples from the sweep file duration
    # Handle both mono and stereo sweep files
    if len(data.shape) > 1:
        recording_length = data.shape[1]  # Stereo: samples in second dimension
    else:
        recording_length = data.shape[0]  # Mono: samples in first dimension
    
    recorder = Thread(
        target=record_target,
        args=(record, recording_length, fs),
        kwargs={'channels': recording_channels, 'append': append}
    )
    recorder.start()
